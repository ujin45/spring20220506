...
DESC Board;
ALTER TABLE Board
ADD COLUMN memberId VARCHAR(20) NOT NULL DEFAULT 'user1'  
REFERENCES Member(id) AFTER body;
...
-- Reply에 memberId 컬럼추가(Member 테이블 id 컬럼 참조키 제약사항, not null 제약사항 추가)
ALTER TABLE Reply
ADD COLUMN memberId VARCHAR(20) NOT NULL DEFAULT 'user1'
...
SELECT r.id, 
			   ...
			   r.inserted,
				IF (m.id = 'user1', 'true', 'false') AS own
